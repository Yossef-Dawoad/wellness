...
import 'package:wellness/features/routine/data/models/get_routine_response.dart';

import '../../../../core/utils/types/networking_error_types.dart';
import '../models/post_routine_request_body.dart';
import '../models/post_routine_response.dart';
...
class RoutineRepo {
  ...
  Future<ApiResult<GetRoutineResponse>> getRoutines() async {
    try {
  ...
  Future<ApiResult<PostRoutineResponse>> addRoutine(
    PostRoutineRequestBody postRoutineRequestBody,
  ) async {
    try {
      final response = await _apiService.addRoutine(postRoutineRequestBody);
      return Right(response);
    } catch (error) {
      final handleError = NetworkErrorHandler.handle(error);
